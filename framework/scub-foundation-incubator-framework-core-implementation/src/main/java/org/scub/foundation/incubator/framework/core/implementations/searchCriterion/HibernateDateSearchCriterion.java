...
public class HibernateDateSearchCriterion extends
    ...
    public void constructHQLCondition(StringBuffer hqlQuery, String columnName) {
        if (validateHqlPreConditions(hqlQuery, columnName)) {
            if (searchCriterion != null && searchCriterion.getOperator() != null) {
                ...
                if (searchCriterion.getOperator().equals(DateOperator.BETWEEN)) {
                    queryPart += constructBetweenQuery(">=", "<=", columnName, AND);
                } else if (searchCriterion.getOperator().equals(DateOperator.NOT_BETWEEN)) {
                    queryPart += constructBetweenQuery("<=", ">=", columnName, OR);
                } else if (searchCriterion.getOperator().equals(DateOperator.STRICTLY_BETWEEN)) {
                    queryPart += constructBetweenQuery(">", "<", columnName, AND);
                } else if (searchCriterion.getOperator().equals(DateOperator.STRICTLY_NOT_BETWEEN)) {
                    queryPart += constructBetweenQuery("<", ">", columnName, OR);
                } else {
                    // all other operators.
    ...
    public void setConditionValue(Query query) {
        if (query != null) {
            if (searchCriterion != null && searchCriterion.getOperator() != null) {
                if (!searchCriterion.getOperator().equals(DateOperator.IS_NOT_NULL) && !searchCriterion.getOperator().equals(DateOperator.IS_NULL)) {
                    if (searchCriterion.getOperator().equals(DateOperator.BETWEEN) || searchCriterion.getOperator().equals(DateOperator.NOT_BETWEEN)
                        || searchCriterion.getOperator().equals(DateOperator.STRICTLY_BETWEEN)
                        || searchCriterion.getOperator().equals(DateOperator.STRICTLY_NOT_BETWEEN)) {
                        final DateSearchCriterionValueDto minBoundaryValue = searchCriterion.getValue();
                        final DateSearchCriterionValueDto maxBoundaryValue = searchCriterion.getSecondValue();
                        ...
                        final DateSearchCriterionValueDto conditionValue = searchCriterion.getValue();
     ...
     */
    public void setQueryParam(Query query, String parameterName, DateSearchCriterionValueDto value) {
        if (query != null && value != null) {
            query.setDate(parameterName, value.getValue());
        }
    }
